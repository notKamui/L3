...
(* point -> point -> bool *)
(* point_domination *)
let point_domination (p1: point) (p2: point): bool = fst p1 >= fst p2 && snd p1 >= snd p2
...
let smallest lst =
    ...
    else List.fold_left (fun acc next -> if compare acc next >= 0 then next else acc) (List.hd lst) (List.tl lst);;
...
let largest lst =
    assert (lst != []);
    if List.length lst = 1 then List.hd lst
    else List.fold_left (fun acc next -> if compare acc next >= 0 then acc else next) (List.hd lst) (List.tl lst);;
...
let quadtree_make (ps: point list) (n: int): quadtree =
    ...
    in aux ps (enclosing_rectangle ps);;

(* Question 13 *)
(* quadtree -> int  *)
(* quadtree_count *)
let rec quadtree_count (tree: quadtree): int = match tree with
    | Leaf(ps, _) -> List.length ps
    | Node(nw, ne, se, sw, _) -> (quadtree_count nw) + (quadtree_count ne) + (quadtree_count se) + (quadtree_count sw);;

(* Question 14 *)
(* quadtree -> int list *)
(* quadtree_signature *)
let rec quadtree_signature (tree: quadtree): int list = match tree with
    | Leaf(ps, _) -> [List.length ps]
    | Node(nw, ne, se, sw, _) -> (quadtree_signature nw) @ (quadtree_signature ne) @ (quadtree_signature se) @ (quadtree_signature sw);;
...
(* quadtree -> point list  *)
(* quadtree_all_points *)
let rec quadtree_all_points (tree: quadtree): point list = match tree with
    | Leaf(ps, _) -> ps
    | Node(nw, ne, se, sw, _) -> (quadtree_all_points nw) @ (quadtree_all_points ne) @ (quadtree_all_points se) @ (quadtree_all_points sw);;
...
let rectangle_intersection (((p1,p2),(p3,p4)): rectangle * rectangle): rectangle=
...
(* rectangle_query *)
let rec rectangle_query (r: rectangle) (tree: quadtree): point list = match tree with
    ...
    | Node(nw, ne, se, sw, root) when not (rectangle_disjoint r root) -> (
        ...
        else (rectangle_query r nw) @ (rectangle_query r ne) @ (rectangle_query r se) @ (rectangle_query r sw)
    )
    | _ -> [] ;;
